...
class Prepare_window():
    ...
    def operating(self):
        ...
        while stay:
            ...
            if bad:
                self.txt6_surf = self.txt_font.render('Please enter your names!', True, red)
                self.txt6_rect = self.txt6_surf.get_rect()
                self.txt6_rect.center = (width / 2, height - 100)
                screen.blit(self.txt6_surf, self.txt6_rect)
    ...
    def ask_name(self, player):
        ...
        if player == 1:
            self.name1 = current_string.capitalize()
        else:
            self.name2 = current_string.capitalize()
